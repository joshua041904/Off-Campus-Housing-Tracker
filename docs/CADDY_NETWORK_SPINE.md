...
- **SNI enforced:** Wrong or missing host → `421 Misdirected Request`. Primary vhost is `https://off-campus-housing.local` only.
...
Requires: `grpcurl`, cluster with Caddy + Envoy, `certs/dev-root.pem` or `dev-root-ca` secret. Uses `grpc.health.v1.Health/Check` via Caddy :443 with authority `off-campus-housing.local`; expects SERVING.
