...
- **Kafka `advertised.listeners` (KRaft + MetalLB):** the **EXTERNAL** listener IP is **not** taken from a static `KAFKA_ADVERTISED_LISTENERS` entry on the StatefulSet. An **init container** (`fetch-external-lb-ip`) reads the broker’s `kafka-N-external` **LoadBalancer** IP from the Kubernetes API and writes it to a **shared volume**; the main container **startup script** reads that file, sets `advertised.listeners` in **`/etc/kafka/kafka.properties`**, then starts Kafka. Preflight: `./scripts/verify-kafka-no-static-advertised-env.sh`, `./scripts/check-kafka-config-drift.sh`; remediation: `./scripts/kafka-sync-metallb.sh`. Optional **Prometheus** drift metric: apply the **kafka-metallb-alignment-exporter** in the `kafka-kraft-metallb` bundle (alert `KafkaMetallbAdvertisedLBDrift`).
