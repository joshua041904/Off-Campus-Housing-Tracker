...
# Output: certs/dev-root.pem, certs/dev-root.key; certs/messaging-service.{crt,key}; certs/media-service.{crt,key};
...
# 2. messaging-service leaf
say "2. Creating messaging-service leaf..."
...
# 3. media-service leaf
say "3. Creating media-service leaf..."
...
# 4. Kafka client cert (Node: KAFKA_SSL_CA_PATH, KAFKA_SSL_CERT_PATH, KAFKA_SSL_KEY_PATH)
say "4. Creating Kafka client leaf (Node)..."
...
# 5. Kafka broker cert (PEM + optional JKS for docker-compose Kafka)
say "5. Creating Kafka broker cert..."
...
if command -v keytool >/dev/null 2>&1; then
  ...
  keytool -importcert -alias dev-root-ca -file "$CA_PEM" -keystore "$KAFKA_SSL/kafka.truststore.jks" -storepass "$PASS" -noprompt 2>/dev/null
