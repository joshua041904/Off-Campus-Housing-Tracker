...
# After mutating tests: fail fast on drift, TLS, advertised.listeners, quorum, etc.
assert_cluster_healthy_post_destructive() {
  ...
  if ! HOUSING_NS="$NS" KAFKA_BROKER_REPLICAS="$REP" bash "$REPO_ROOT/scripts/kafka-runtime-sync.sh" --check-only "$NS" "$REP"; then
    bad "Post-destructive: kafka-runtime-sync --check-only failed ($tag)"
    return 1
  fi
  if ! VERIFY_KAFKA_HEALTH_ONLY=0 \
    ...
    bad "Post-destructive: verify-kafka-cluster failed ($tag)"
    return 1
  fi
