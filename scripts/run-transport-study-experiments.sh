...
if _has_exp 4; then
  ...
  if [[ "${TRANSPORT_STUDY_RUN_EXP4:-1}" == "1" ]] && command -v kubectl >/dev/null 2>&1; then
    ...
    if [[ -n "$_node_ip" ]] && [[ -n "$_np" ]] && [[ "$_node_ip" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      _code=$(curl -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 3 --max-time 8 "https://${_node_ip}:${_np}/_caddy/healthz" -H "Host: record.local" 2>/dev/null || echo "000")
      if [[ "$_code" == "200" ]]; then
...
if _has_exp 5; then
  ...
  if [[ "${TRANSPORT_STUDY_RUN_EXP5:-1}" == "1" ]]; then
    ...
    if command -v caddy >/dev/null 2>&1; then
      ...
      info "  Point k6: BASE_URL=https://localhost:443 K6_RESOLVE=record.local:443:127.0.0.1"
