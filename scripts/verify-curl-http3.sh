...
echo ""
echo "=== Minimum version (bootstrap / make deps) ==="
echo "  This repo enforces curl >= ${MIN_CURL_VERSION:-8.19.0} with HTTP/3 via scripts/check-curl-preflight-reqs.sh"
echo "  (SKIP_CURL_MIN_VERSION=1 skips only the semver gate; HTTP/3 is still required unless SKIP_CURL_HTTP3_CHECK=1)."
