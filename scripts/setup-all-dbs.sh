#!/usr/bin/env bash
# Run all infra/db schema scripts so every housing DB (ports 5442–5447) is set up correctly.
# Matches infra/db/README.md: listings (5442), bookings (5443), messaging (5444),
# notification (5445), trust (5446), analytics (5447). Auth (5441) is Prisma + optional
...
#
# Optional: DO_DOCKER_UP=1 to start all 7 Postgres containers before applying schemas.
# Prereq: psql on PATH (or ensure scripts will fail clearly). Ports 5441–5447.
...
if [[ "${DO_DOCKER_UP:-0}" == "1" ]]; then
  say "Starting all 7 Postgres containers (ports 5441–5447)..."
  docker compose up -d postgres-auth postgres-listings postgres-bookings postgres-messaging postgres-notification postgres-trust postgres-analytics 2>/dev/null || true
...
say "=== All DBs set up ==="
echo ""
echo "Ports 5442–5447: schema applied (listings, bookings, messaging, notification, trust, analytics)."
echo "Port 5441 (auth): Prisma/restore only — run restore-auth-from-legacy-dump.sh if needed."
echo ""
echo "Optional: PGPASSWORD=postgres ./scripts/restore-good-db-settings.sh to apply tuning to all 7 instances."
