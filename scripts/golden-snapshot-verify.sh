...
#   GOLDEN_SNAPSHOT_CHAOS=1 ./scripts/golden-snapshot-verify.sh # append make chaos-suite-kafka (destructive)
...
if [[ "${GOLDEN_SNAPSHOT_CHAOS:-0}" == "1" ]]; then
  ...
  warn "GOLDEN_SNAPSHOT_CHAOS not set — skip chaos-suite-kafka (set GOLDEN_SNAPSHOT_CHAOS=1 to run)"
fi
