...
for attempt in $(seq 1 "$HAPROXY_ATTEMPTS"); do
  ...
  if [[ "$CODE" == "503" ]]; then
    ...
    if [[ $consecutive_503 -ge "$HAPROXY_CONSECUTIVE_503_FAIL" ]]; then
      info "  Diagnose (HAProxy pod has no curl): kubectl run curl-diagnose --rm -i --restart=Never -n $NS_APP --image=curlimages/curl:latest -- curl -s -H 'x-traffic-class: internal' -o /dev/null -w '%{http_code}' http://api-gateway.${NS_APP}.svc.cluster.local:4020/readyz"
