# EXPLAIN ANALYZE — all housing databases

Generated: 2026-03-31T19:18:21Z
Host: `127.0.0.1` user: `postgres`

| Service | Port | Database | SQL file |
|---------|------|----------|----------|
| auth | 5441 | auth | explain-auth.sql |
| listings | 5442 | listings | explain-listings.sql |
| booking | 5443 | bookings | explain-bookings.sql |
| messaging | 5444 | messaging | explain-messaging.sql |
| notification | 5445 | notification | explain-notification.sql |
| trust | 5446 | trust | explain-trust.sql |
| analytics | 5447 | analytics | explain-analytics.sql |
| media | 5448 | media | explain-media.sql |


## auth (port 5441, database `auth`)

```
                                                            QUERY PLAN                                                             
-----------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=0.28..8.29 rows=1 width=56) (actual time=1.128..1.129 rows=0 loops=1)
   Output: id, email
   Buffers: shared hit=5
   ->  Index Scan using idx_users_email on auth.users  (cost=0.28..8.29 rows=1 width=56) (actual time=1.127..1.127 rows=0 loops=1)
         Output: id, email
         Index Cond: (users.email = '__explain_probe_nonexistent__@example.com'::citext)
         Buffers: shared hit=5
 Planning:
   Buffers: shared hit=110
 Planning Time: 8.543 ms
 Execution Time: 4.826 ms
(11 rows)

```

## listings (port 5442, database `listings`)

```
                                                                   QUERY PLAN                                                                   
------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=14.57..14.57 rows=1 width=134) (actual time=0.520..0.522 rows=8 loops=1)
   Output: id, user_id, title, description, price_cents, amenities, smoke_free, pet_friendly, furnished, ((status)::text), created_at
   Buffers: shared hit=7
   ->  Sort  (cost=14.57..14.57 rows=1 width=134) (actual time=0.519..0.520 rows=8 loops=1)
         Output: id, user_id, title, description, price_cents, amenities, smoke_free, pet_friendly, furnished, ((status)::text), created_at
         Sort Key: listings.created_at DESC
         Sort Method: quicksort  Memory: 26kB
         Buffers: shared hit=7
         ->  Seq Scan on listings.listings  (cost=0.00..14.56 rows=1 width=134) (actual time=0.071..0.074 rows=8 loops=1)
               Output: id, user_id, title, description, price_cents, amenities, smoke_free, pet_friendly, furnished, (status)::text, created_at
               Filter: ((listings.deleted_at IS NULL) AND ((listings.status)::text = 'active'::text))
               Buffers: shared hit=4
 Planning:
   Buffers: shared hit=428
 Planning Time: 2.384 ms
 Execution Time: 0.573 ms
(16 rows)

                                                                                                  QUERY PLAN                                                                                                   
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=15.87..15.87 rows=1 width=134)
   Output: id, user_id, title, description, price_cents, amenities, smoke_free, pet_friendly, furnished, ((status)::text), created_at
   ->  Sort  (cost=15.87..15.87 rows=1 width=134)
         Output: id, user_id, title, description, price_cents, amenities, smoke_free, pet_friendly, furnished, ((status)::text), created_at
         Sort Key: listings.created_at DESC
         ->  Seq Scan on listings.listings  (cost=0.00..15.86 rows=1 width=134)
               Output: id, user_id, title, description, price_cents, amenities, smoke_free, pet_friendly, furnished, (status)::text, created_at
               Filter: ((listings.deleted_at IS NULL) AND ((listings.title ~~* '%explain-probe%'::text) OR (listings.description ~~* '%explain-probe%'::text)) AND ((listings.status)::text = 'active'::text))
 Planning:
   Buffers: shared hit=1
(10 rows)

```

## booking (port 5443, database `bookings`)

```
                                                       QUERY PLAN                                                        
-------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=3.91..4.04 rows=50 width=76) (actual time=0.459..0.465 rows=50 loops=1)
   Output: id, listing_id, tenant_id, landlord_id, status, created_at
   Buffers: shared hit=5
   ->  Sort  (cost=3.91..4.04 rows=50 width=76) (actual time=0.458..0.460 rows=50 loops=1)
         Output: id, listing_id, tenant_id, landlord_id, status, created_at
         Sort Key: bookings.created_at DESC
         Sort Method: quicksort  Memory: 31kB
         Buffers: shared hit=5
         ->  Seq Scan on booking.bookings  (cost=0.00..2.50 rows=50 width=76) (actual time=0.019..0.412 rows=50 loops=1)
               Output: id, listing_id, tenant_id, landlord_id, status, created_at
               Buffers: shared hit=2
 Planning:
   Buffers: shared hit=266 dirtied=6
 Planning Time: 11.370 ms
 Execution Time: 0.536 ms
(15 rows)

```

## messaging (port 5444, database `messaging`)

```
                                                         QUERY PLAN                                                          
-----------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=6.49..6.62 rows=50 width=78) (actual time=1.816..1.823 rows=50 loops=1)
   Output: id, conversation_id, sender_id, body, created_at
   Buffers: shared hit=5 dirtied=1
   ->  Sort  (cost=6.49..6.75 rows=104 width=78) (actual time=1.814..1.817 rows=50 loops=1)
         Output: id, conversation_id, sender_id, body, created_at
         Sort Key: messages.created_at DESC
         Sort Method: top-N heapsort  Memory: 38kB
         Buffers: shared hit=5 dirtied=1
         ->  Seq Scan on messaging.messages  (cost=0.00..3.04 rows=104 width=78) (actual time=0.204..1.075 rows=102 loops=1)
               Output: id, conversation_id, sender_id, body, created_at
               Filter: (messages.deleted_at IS NULL)
               Buffers: shared hit=2 dirtied=1
 Planning:
   Buffers: shared hit=121
 Planning Time: 10.792 ms
 Execution Time: 1.961 ms
(16 rows)

                                                                            QUERY PLAN                                                                            
------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=4.13..11.24 rows=3 width=40) (actual time=0.284..0.285 rows=0 loops=1)
   Output: conversation_id, user_id, joined_at
   Buffers: shared hit=2
   ->  Bitmap Heap Scan on messaging.conversation_participants  (cost=4.13..11.24 rows=3 width=40) (actual time=0.283..0.283 rows=0 loops=1)
         Output: conversation_id, user_id, joined_at
         Recheck Cond: ((conversation_participants.user_id = '00000000-0000-0000-0000-000000000001'::uuid) AND (NOT conversation_participants.deleted))
         Buffers: shared hit=2
         ->  Bitmap Index Scan on idx_conversation_participants_user_archived_deleted  (cost=0.00..4.13 rows=3 width=0) (actual time=0.279..0.280 rows=0 loops=1)
               Index Cond: (conversation_participants.user_id = '00000000-0000-0000-0000-000000000001'::uuid)
               Buffers: shared hit=2
 Planning:
   Buffers: shared hit=72
 Planning Time: 2.316 ms
 Execution Time: 0.343 ms
(14 rows)

```

## notification (port 5445, database `notification`)

```
                                                             QUERY PLAN                                                              
-------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=32.91..33.03 rows=50 width=76) (actual time=1.748..1.753 rows=26 loops=1)
   Output: id, user_id, event_type, status, created_at
   Buffers: shared hit=6 dirtied=2
   ->  Sort  (cost=32.91..34.23 rows=530 width=76) (actual time=1.746..1.749 rows=26 loops=1)
         Output: id, user_id, event_type, status, created_at
         Sort Key: notifications.created_at DESC
         Sort Method: quicksort  Memory: 28kB
         Buffers: shared hit=6 dirtied=2
         ->  Seq Scan on notification.notifications  (cost=0.00..15.30 rows=530 width=76) (actual time=0.797..1.282 rows=26 loops=1)
               Output: id, user_id, event_type, status, created_at
               Buffers: shared hit=3 dirtied=2
 Planning:
   Buffers: shared hit=160
 Planning Time: 11.571 ms
 Execution Time: 1.886 ms
(15 rows)

```

## trust (port 5446, database `trust`)

```
                                                         QUERY PLAN                                                          
-----------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=29.02..29.14 rows=50 width=60) (actual time=1.397..1.400 rows=4 loops=1)
   Output: id, listing_id, reporter_id, status, created_at
   Buffers: shared hit=4
   ->  Sort  (cost=29.02..30.12 rows=440 width=60) (actual time=1.395..1.396 rows=4 loops=1)
         Output: id, listing_id, reporter_id, status, created_at
         Sort Key: listing_flags.created_at DESC
         Sort Method: quicksort  Memory: 25kB
         Buffers: shared hit=4
         ->  Seq Scan on trust.listing_flags  (cost=0.00..14.40 rows=440 width=60) (actual time=0.567..0.870 rows=4 loops=1)
               Output: id, listing_id, reporter_id, status, created_at
               Buffers: shared hit=1
 Planning:
   Buffers: shared hit=151
 Planning Time: 33.164 ms
 Execution Time: 1.652 ms
(15 rows)

```

## analytics (port 5447, database `analytics`)

```
                                                                 QUERY PLAN                                                                  
---------------------------------------------------------------------------------------------------------------------------------------------
 Index Scan using daily_metrics_pkey on analytics.daily_metrics  (cost=0.15..8.17 rows=1 width=28) (actual time=1.944..1.949 rows=1 loops=1)
   Output: date, new_users, new_listings, new_bookings, completed_bookings, messages_sent, listings_flagged
   Index Cond: (daily_metrics.date = CURRENT_DATE)
   Buffers: shared hit=2
 Planning:
   Buffers: shared hit=77
 Planning Time: 14.177 ms
 Execution Time: 2.407 ms
(8 rows)

                                                     QUERY PLAN                                                     
--------------------------------------------------------------------------------------------------------------------
 HashAggregate  (cost=17.20..19.20 rows=200 width=40) (actual time=0.010..0.011 rows=0 loops=1)
   Output: event_type, count(*)
   Group Key: events.event_type
   Batches: 1  Memory Usage: 40kB
   ->  Seq Scan on analytics.events  (cost=0.00..14.80 rows=480 width=32) (actual time=0.007..0.008 rows=0 loops=1)
         Output: id, event_type, event_version, payload, source_service, created_at, event_id
 Planning:
   Buffers: shared hit=122
 Planning Time: 13.214 ms
 Execution Time: 1.310 ms
(10 rows)

```

## media (port 5448, database `media`)

```
                                                        QUERY PLAN                                                         
---------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=4.47..4.59 rows=50 width=131) (actual time=0.694..0.703 rows=50 loops=1)
   Output: id, user_id, object_key, status, created_at
   Buffers: shared hit=5
   ->  Sort  (cost=4.47..4.62 rows=62 width=131) (actual time=0.693..0.696 rows=50 loops=1)
         Output: id, user_id, object_key, status, created_at
         Sort Key: media_files.created_at DESC
         Sort Method: quicksort  Memory: 42kB
         Buffers: shared hit=5
         ->  Seq Scan on media.media_files  (cost=0.00..2.62 rows=62 width=131) (actual time=0.382..0.447 rows=68 loops=1)
               Output: id, user_id, object_key, status, created_at
               Buffers: shared hit=2
 Planning:
   Buffers: shared hit=154
 Planning Time: 12.858 ms
 Execution Time: 0.914 ms
(15 rows)

```

---
End of EXPLAIN section.
