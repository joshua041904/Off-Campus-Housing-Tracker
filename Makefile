...
dev-onboard-lite: ## CI-safe: bash -n scripts, kustomize kafka bundle, onboard script wiring (avoids full make -n tree)
	...
	bash -n "$(SCRIPTS)/ci/shell-static-checks.sh"; \
	echo "✅ dev-onboard-lite OK"

.PHONY: verify-ci
verify-ci: ## CI gate (GitHub kustomize-manifest-static): kustomize + bash -n — same contract as dev-onboard-lite
	@chmod +x "$(SCRIPTS)/ci/shell-static-checks.sh"
	@bash "$(SCRIPTS)/ci/shell-static-checks.sh"
...
observe: ensure-node20 ## Phase-vi2 skew + analytics QA + k6 transport load; prints Jaeger/Grafana/Prometheus URLs (best-effort steps use || true)
	...
	cd "$(REPO_ROOT)" && BASE_URL="$${BASE_URL:-https://off-campus-housing.test}" ANALYTICS_QA_TLS_INSECURE="$${ANALYTICS_QA_TLS_INSECURE:-1}" node scripts/analytics-qa/run-all.mjs || true
	@echo "=== Generating Load (k6 h2) ==="
	cd "$(REPO_ROOT)" && k6 run scripts/load/k6-transport-h2.js || true
	@echo ""
	@echo "Open:"
	@echo "  https://off-campus-housing.test/jaeger"
	@echo "  https://off-campus-housing.test/grafana  (dashboard: AI Intelligence – Analytics + Kafka + QA)"
	@echo "  https://off-campus-housing.test/prometheus"
	@echo ""
	@echo "To push QA duration + skew gauges into analytics /metrics: set on analytics-service ANALYTICS_TELEMETRY_INGEST=1 and ANALYTICS_TELEMETRY_TOKEN; then export ANALYTICS_TELEMETRY_BASE_URL=https://off-campus-housing.test/api/analytics ANALYTICS_TELEMETRY_TOKEN=… for the QA runner."
	@echo "Optional browser load: pnpm run load:ui-analytics (Playwright 04-analytics; see docs/AI_SRE_HANDBOOK.md)."
