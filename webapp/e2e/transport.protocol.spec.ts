...
test.describe("transport protocol (curl)", () => {
  ...
  test("HTTP/3-only must negotiate version 3 (downgrade fails)", async ({ request }) => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        const connectT = attempt === 2 ? "20" : "15";
        const maxT = attempt === 2 ? "35" : "30";
        v = curlVersion(["--http3-only", "--connect-timeout", connectT, "--max-time", maxT]);
        lastErr = undefined;
        break;
      } catch (e) {
        lastErr = e;
        if (attempt < 2) await delay(1000 + attempt * 500);
        else if (process.env.PLAYWRIGHT_STRICT_HTTP3 === "1") throw e;
        else {
    ...
    if (lastErr !== undefined && v === "") throw lastErr;
