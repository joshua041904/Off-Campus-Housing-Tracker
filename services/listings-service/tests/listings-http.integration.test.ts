...
import {
  sumTopicHighWatermarks,
  waitForKafkaTopicHighBeyond,
} from "@common/utils";
...
describe.skipIf(!dbReady)("listings HTTP — Postgres integration", () => {
  ...
  it("GET / (root search) returns 200 with items array", async () => {
    const res = await request(app)
      .get("/")
      .query({ q: "integration-root-search-no-match" });
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.items)).toBe(true);
  });

  it("POST /create returns 201 and row is readable via GET and SQL", async () => {
    ...
    try {
      const row = await verify.query(
        "SELECT id FROM listings.listings WHERE id = $1::uuid AND deleted_at IS NULL",
        [id],
      );
  ...
  it("GET /listings/:id returns 404 for valid uuid that does not exist", async () => {
    const missingId = "11111111-1111-4111-8111-111111111111";
    const res = await request(app).get(`/listings/${missingId}`);
  ...
  it("GET /search returns items array", async () => {
    const res = await request(app)
      .get("/search")
      .query({ q: "zzzz-no-match-integration" });
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.items)).toBe(true);
  });
});
